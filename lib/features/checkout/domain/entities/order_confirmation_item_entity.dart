...
class OrderConfirmationItemMessageEntity extends Equatable {
  final String? alertMessage;
  final bool hasIcon;
  final String? messageType;
  final String? actionLink;

  const OrderConfirmationItemMessageEntity({
    this.alertMessage,
    this.hasIcon = false,
    this.messageType,
    this.actionLink,
...
/// Order-confirmation product line item — a flat `{price, regularPrice,
/// discount, ...}` shape distinct from [CartItemEntity], which moved to a
/// nested backend shape (`priceInfo`, `media`, etc.) that order-confirmation
/// doesn't send.
class OrderConfirmationItemEntity extends Equatable {
  final int? productId;
  final String? sku;
  ...
  final OrderConfirmationItemMessageEntity? message;
  final int? orderPrice;

  const OrderConfirmationItemEntity({
    this.productId,
    this.sku,
  ...
  bool get isCompletelySoldOut => (isSoldOut == true) || (isSizeSoldOut == true);
