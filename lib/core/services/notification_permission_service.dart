...
/// Android-only notification-permission nudge orchestrator. Ports Android's
/// `NotificationUtil` + the per-screen `CTPushPermissionNotificationResponseListener`
/// wiring: gates *when* to ask for `POST_NOTIFICATIONS` per screen using the
...
class NotificationPermissionService {
  ...
  Future<void> _requestOsPermission(String fromScreen) async {
    ...
    await CleverTapPlugin.promptForPushNotification(true);
  }
  ...
  Future<bool> shouldShowHomepageDialog({bool isFromOrderConfirmation = false}) async {
    if (!_isSupportedPlatform || await isGranted) return false;
  ...
  Future<bool> shouldShowWebviewNudge() async {
    if (!_isSupportedPlatform || _nudgeHelper.getNudge(_wishlistScreen) == null) return false;
