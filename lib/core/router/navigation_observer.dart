...
class AppNavigationObserver extends NavigatorObserver {
  ...
  static const Map<String, Funnel> _funnelRoutes = {
    ...
    RouteNames.wishlistName: Funnel.wishlist,
  };
  ...
  static const Map<String, String> _routeToScreenLabel = {
    ...
    RouteNames.wishlistName: FromScreens.wishlist,
    // Auth, so a post-login event can see the sign-in detour. Without these
    // the trail jumps straight from the surface that prompted the login to
    // whatever follows it, and nine device captures showed exactly that:
    // every auth payload read ["Account", "Discover", "Splash"].
    //
    // These populate `nav_screens` ONLY. The auth events take `from_screen`
    // from AuthEntryArgs, and nothing derives a screen from the trail, so
    // adding them cannot change any event's from_screen.
    //
    // checkoutLogin is here because it builds the same LoginPage — the
    // checkout sign-in is a detour worth seeing too.
    RouteNames.login: FromScreens.login,
    RouteNames.checkoutLogin: FromScreens.login,
    RouteNames.joinUs: FromScreens.join,
    RouteNames.otpVerification: FromScreens.otpVerification,
  };
