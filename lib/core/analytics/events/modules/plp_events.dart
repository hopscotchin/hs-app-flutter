...
/// ## Known divergence
///
/// Android passes a second dispatch flag, `addUniversalProperties`, which
/// appends a `universal` property (the pending common-properties list, or
/// `"none"`). Flutter's [logEvent] has no such flag, so **`universal` is not
/// sent on any event here.** Documented per-event as `universal ✗ (Flutter)`.
extension PlpEvents on AnalyticsHelper {
  ...
  /// Flags: `attribution ✓` · `universal ✗ (Flutter)`.
  ///
  ...
  /// **`attribution ✗`**. Both `universal ✗ (Flutter)`. The asymmetry is
  ...
  ///
  /// Flags: `attribution ✓` · `universal ✗ (Flutter)`. Dispatched via
