...
class ProductGridWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (gridData.rows.isEmpty) return const SizedBox.shrink();
    ...
    return Column(
      ...
      children: [
        ...
        // Product rows with horizontal padding
        Padding(
        ...
        if (gridData.cta != null)
          Padding(
            ...
            child: _buildCta(context, gridData.cta!),
          ),
      ],
    );
  }
  ...
  Widget _buildProductCard(BuildContext context, ProductGridItem item) {
    final showInfo = gridData.layoutInfo?.showProductInfo ?? true;
    return ProductTile.fromGridItem(
      item,
      showProductInfo: showInfo,
      onTap: () =>
          ActionUrlHandler.navigate(context, item.actionUrl, title: item.name),
    );
  }

  Widget _buildCta(BuildContext context, CtaData cta) {
    return Center(
      child: TextButton(
        onPressed: () => ActionUrlHandler.navigate(context, cta.actionUrl),
        child: Text(
          cta.text ?? DiscoverStrings.viewAll,
