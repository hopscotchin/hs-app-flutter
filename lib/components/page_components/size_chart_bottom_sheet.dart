...
import '../../features/pdp/domain/entities/size_chart_entity.dart';
...
/// Size-chart sheet chrome (header + close), shared by PDP and wishlist.
///
/// The chart data itself is fetched per feature, so [bodyBuilder] supplies the
/// state-driven body — typically a `BlocBuilder` returning [SizeChartStateView].
///
/// Pushed on the root navigator as a non-opaque route, so a size-selection
/// sheet underneath stays mounted and reappears (with its selection) when the
/// chart is closed.
void showSizeChartSheet(
...
class _SizeChartSheet extends StatelessWidget {
  const _SizeChartSheet({this.productName, required this.bodyBuilder});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        // Scrollable body
        Expanded(child: Builder(builder: bodyBuilder)),
      ],
    );
  }
}

/// Loading / error / loaded rendering of a [SizeChartEntity]. Kept separate
/// from the sheet chrome so each feature can drive it from its own Bloc.
class SizeChartStateView extends StatelessWidget {
  const SizeChartStateView({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    if (chart != null) {
      if (chart!.charts.isEmpty) {
        return const Center(child: Text('No size chart available.'));
      }
      return ListView.builder(
        padding: EdgeInsets.only(bottom: 24 + bottomInset),
        itemCount: chart!.charts.length,
        itemBuilder: (_, i) =>
            _SizeChartDtoView(dto: chart!.charts[i], chartIndex: i),
      );
    }
