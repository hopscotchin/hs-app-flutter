...
/// `reco_product_clicked` — a recommendations tile was tapped.
///
...
void main() {
  ...
  Future<void> fire() =>
      c.h.analytics.logRecoProductClicked(product: c.product, tile: c.railTile);

  test('fires once, carrying the product node', () async {
    await fire();
    expectProductNode(c.single(AnalyticsEvents.recoProductClicked));
  });

  test('adds exactly the prefixed click block', () async {
    await fire();
    expectAddsExactly(c.single(AnalyticsEvents.recoProductClicked), clickedProductKeys);
  });

  test('the click block describes the TAPPED product', () async {
    await fire();
    final e = c.single(AnalyticsEvents.recoProductClicked);
