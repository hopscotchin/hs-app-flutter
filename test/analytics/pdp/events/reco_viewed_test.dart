...
/// `reco_viewed` — the recommendations rail came into view.
///
...
void main() {
  ...
  Future<void> fire() =>
      c.h.analytics.logRecoViewed(product: c.product, railTrackingMeta: c.railMeta);

  test('fires once, carrying the product node', () async {
    await fire();
    expectProductNode(c.single(AnalyticsEvents.recoViewed));
  });

  test('adds only feed_size, from the rail node', () async {
    await fire();
    final e = c.single(AnalyticsEvents.recoViewed);
  ...
  test('carries no tile identity — nothing was clicked', () async {
    await fire();
    expectAbsent(c.single(AnalyticsEvents.recoViewed), clickedProductKeys);
  });

  test('with no rail node the event still fires, minus feed_size', () async {
    await c.h.analytics.logRecoViewed(product: c.product);
    expectAddsExactly(c.single(AnalyticsEvents.recoViewed), const <String>{});
  });
}
