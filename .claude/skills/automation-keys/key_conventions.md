...
5. **Update the QA doc** — mirror every added/changed/removed key into
   `lib/core/constants/strings/AUTOMATION_KEYS.md` in the same change (see §8).
6. Don't restate keys the user must request — this is default behaviour.

---

## 8. QA reference doc — `AUTOMATION_KEYS.md`

`lib/core/constants/strings/AUTOMATION_KEYS.md` is the human/QA-facing list of
every key, grouped by screen, shared to QA and pushed to GitHub (renders as
tables). It is a **mirror** of `auto_test_strings.dart` — the Dart file stays the
source of truth; the doc must never drift.

Table schema per screen section: **`| Type | Key | Widget file |`** — and for
sections with dynamic keys, a 4th `Examples` column (`| Type | Key (pattern) |
Examples | Widget file |`).

- **Type** — the widget role, capitalised: `Button`, `Text field`, `Hint`,
  `Suffix`, `Text` (asserted `Text`), `Title`, `Image`, `Checkbox`, `List item`,
  `Nav item`, `Tab`, `Chip`, `Option`, `Row`. Match the nearest existing row.
- **Key** — the literal key. Dynamic keys show the `_<i>` pattern (`plp_tile_<i>`),
  never a literal index; put 1–2 concrete examples (`plp_tile_0`, `plp_tile_1`) in
  the Examples column. Static keys get `—` in Examples.
- **Widget file** — basename of the `.dart` file the key is attached in (find it by
  grepping the constant, e.g. `grep -rl PlpTestStrings.tile lib`). If a key is
  defined but not yet wired to a widget, write `*(defined, not yet wired)*`.
- Nested sub-CTAs get their own row (`plp_tile_<i>_wishlist`).
- New screen → add a new `## <Screen>` section (with a `File:` line if single-file)
  and table, placed to match the class order in the Dart file.
- Keep the top "How to read a key" section in sync if a new element/suffix role is
  introduced (mirror §2's suffix table).

The doc is regenerated by hand from the Dart file — if they ever disagree, the
Dart file wins; fix the doc.